...
const LEARNING_PATH=[{topic:'Perfekt revision',focus:'Perfekt + haben/sein + Partizip II'},{topic:'Word order',focus:'Verb in Position 2 + time first inversion'},{topic:'Cases',focus:'Nominativ / Akkusativ / Dativ'},{topic:'Articles',focus:'der / den / dem • ein / einen / einem'},{topic:'Prepositions',focus:'fixed Dativ / Akkusativ + Wechselpräpositionen'},{topic:'Reflexive verbs',focus:'mich / dich / sich + Perfekt'},{topic:'Verb + preposition',focus:'warten auf / denken an / sprechen mit'},{topic:'Imperative',focus:'du / ihr / Sie forms'},{topic:'um...zu / damit',focus:'same subject vs different subject'},{topic:'werden',focus:'future / passive / become'},{topic:'Modal verbs',focus:'können / müssen / dürfen / sollen / wollen'},{topic:'Nebensätze',focus:'weil / dass / wenn / obwohl / nachdem'},{topic:'Separable verbs',focus:'prefix placement under pressure'},{topic:'Question forms',focus:'W-questions + yes/no'},{topic:'Adjective endings',focus:'after definite and indefinite articles'},{topic:'Relative clauses',focus:'der / die / das as relative pronouns'},{topic:'Passive',focus:'werden + Partizip II'},{topic:'Konjunktiv II',focus:'würde / hätte / wäre + polite hypotheticals'}];
...
function switchPracticeGroup(name){
  ...
  if(name==='grammar'){ if(!['exercises','cases','fixed','topicflow'].includes(state.practice)) switchPractice('topicflow'); else if(!state.currentTopicFlow && state.practice==='topicflow') newTopicFlow(); }
  if(name==='production'){ if(!['typing','structure','writing'].includes(state.practice)) switchPractice('structure'); }
  if(name==='listening'){
    state.practice='listening'; store.lastPractice='listening';
    const panel=$('#practice-listening'); if(panel) panel.classList.remove('hidden');
    if(!state.currentListen) newListening(); else newListeningFromState();
  }
  if(name==='revision'){
    state.practice='revision'; store.lastPractice='revision';
    const panel=$('#practice-revision'); if(panel) panel.classList.remove('hidden');
    renderRevisionPanel();
    if(!state.currentRevision) newRevisionMix();
  }
}
...
function buildListeningPool(){
  const fromPassages=(state.data.passages||[]).slice(0,24).map((p,idx)=>({
    level:p.level||'A2',
    text:p.german||'',
    question_en:'What is the topic of the text?',
    question_de:'Worum geht es im Text?',
    answer:p.title || p.chapter || p.topic || 'Der Text handelt von einem Alltagsthema.',
    reason_en:'A short topic answer is enough here.',
    reason_de:'Hier reicht eine kurze Antwort zum Thema.'
  })).filter(x=>x.text);
  return [...LISTENING_BANK, ...fromPassages];
}
function getRevisionPool(topics=[]){
  const ex=(state.data.exercises||[]).map(q=>({question:q.question,answer:q.answer,topic:q.topic||q.type||'General',level:q.level||'A1',explanation:q.explanation||''}));
  const fixed=(FIXED_RULES_BANK||[]).map(q=>({question:q.question,answer:q.answer,topic:q.topic||'Fixed Rules',level:'A2',explanation:q.explanation||q.rule||''}));
  const topic=(TOPIC_FLOW_BANK||[]).map(q=>({question:q.question,answer:q.answer,topic:q.topic||'Topic Lessons',level:q.level||'B1',explanation:q.explanation||q.intro||''}));
  const casep=(CASE_DRILLS||[]).map(q=>({question:q.question||q.prompt,answer:q.answer,topic:q.topic||'Cases',level:q.level||'A2',explanation:q.reason||q.explanation||''}));
  let pool=[...ex,...fixed,...topic,...casep].filter(x=>x.question&&x.answer);
  if(topics.length) pool=pool.filter(e=>topics.includes(e.topic)||topics.includes(e.type));
  return dedupeBy(pool,x=>`${x.topic}|${x.level}|${x.question}|${x.answer}`);
}
function labelSection(sec){ return ({home:t('Home','Start'),learn:t('Learn','Lernen'),practice:t('Practice','Üben'),exam:t('Exam','Prüfung'),progress:t('Progress','Fortschritt'),settings:t('Settings','Einstellungen'),vocab:t('Vocabulary','Wortschatz'),grammar:t('Grammar','Grammatik'),flashcards:t('Flashcards','Karteikarten'),weak:t('Weak Areas','Schwächen'),conjugation:t('Conjugation','Konjugation'),lid:'Leben in Deutschland',app:'App'})[sec]||sec; }
...
function renderRevisionPanel(){
  const host=$('#revisionTopicGrid'); if(!host) return;
  const topics=[...new Set(getRevisionPool([]).map(x=>x.topic).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
...
function startRevisionMix(){ const topics=selectedRevisionTopics(); state.revisionPool=shuffle(getRevisionPool(topics)); newRevisionMix(); }
function newRevisionMix(){ const pool=state.revisionPool?.length?state.revisionPool:shuffle(getRevisionPool([])); const q=pool[0]; state.currentRevision=q||null; if(!q){ $('#revisionQuestion').textContent='No revision items found.'; const m=$('#revisionMeta'); if(m) m.classList.add('hidden'); return; } const m=$('#revisionMeta'); if(m) m.classList.remove('hidden'); if(m) m.textContent=`${q.level||'A1'} • ${q.topic||q.type||'General'}`; $('#revisionQuestion').textContent=q.question; $('#revisionInput').value=''; $('#revisionFeedback').textContent=''; $('#revisionExplanation').classList.add('hidden'); }
...
function goSection(sec){ state.section=sec; store.lastSection=sec; saveStore(); $$('.app-section').forEach(s=>s.classList.add('hidden')); const node=$('#section-'+sec); if(node) node.classList.remove('hidden'); $$('.navbtn').forEach(b=>b.classList.toggle('active',b.dataset.section===sec)); $('#topTitle').textContent=labelSection(sec); $('#sidebar').classList.remove('open'); if(sec==='lid'){ $('#lidShowEnglish').checked=false; newLid(); } if(sec==='practice'){ if($('#listenShowEnglish')) $('#listenShowEnglish').checked=false; if($('#writingShowEnglish')) $('#writingShowEnglish').checked=false; if($('#qaShowEnglish')) $('#qaShowEnglish').checked=false; if(state.practiceGroup==='listening'){ if(!state.currentListen) newListening(); else newListeningFromState(); } else if(state.practiceGroup==='revision'){ renderRevisionPanel(); if(!state.currentRevision) newRevisionMix(); } else if(state.practice==='structure') newStructure(); } if(sec==='grammar') renderGrammar(); if(sec==='app'||sec==='settings') renderApp(); if(sec==='vocab') renderVocabList(); if(sec==='home') renderHomeSection(); if(sec==='learn') renderLearnSection(); if(sec==='progress') renderProgressSection(); }
...
function newListening(){ const lvl=$('#listenLevelFilter').value; let pool=buildListeningPool().filter(x=>lvl==='ALL'||x.level===lvl); if(!pool.length) pool=buildListeningPool(); state.currentListen=pool[Math.floor(Math.random()*pool.length)]||buildListeningPool()[0]; if(!state.currentListen){ $('#listenPrompt').textContent='No listening item available.'; return; } newListeningFromState(); }
...
function renderHomeSection(){
  ...
  if($('#homeStreak')) $('#homeStreak').textContent=store.streak||0;
  if($('#homeXP')) $('#homeXP').textContent=store.xp||0;
  if($('#homeDue')) $('#homeDue').textContent=dueReviews().length;
  if($('#homeTodayPlan')) $('#homeTodayPlan').innerHTML=plan.map(x=>`<div>• ${x}</div>`).join('');
  if($('#homeNextStep')) $('#homeNextStep').innerHTML=weak[0]?`<strong>${t('Focus next','Nächstes Thema')}:</strong> ${weak[0].type}`:`<strong>${t('Next topic','Nächstes Thema')}:</strong> ${LEARNING_PATH[0].topic}`;
  if($('#homeRoadmap')){
    $('#homeRoadmap').innerHTML=LEARNING_PATH.map((x,i)=>`<button class="roadmap-item ${i<4?'done':''}" data-topicopen="${x.topic}"><strong>${i+1}. ${x.topic}</strong><span>${x.focus}</span></button>`).join('');
    $$('#homeRoadmap [data-topicopen]').forEach(b=>b.onclick=()=>{ goSection('practice'); switchPracticeGroup('grammar'); switchPractice('topicflow'); });
  }
}
function switchLearn(mode){ $$('.learnbtn').forEach(b=>b.classList.toggle('active', b.dataset.learn===mode)); $$('.learn-panel').forEach(p=>p.classList.add('hidden')); const panel=$('#learn-'+mode); if(panel) panel.classList.remove('hidden'); if(mode==='sequence') renderLearnSequence(); if(mode==='grammarlib') renderLearnGrammar(); if(mode==='vocablib') renderLearnVocab(); if(mode==='verblib') renderLearnVerbs(); }
function renderLearnSection(){ renderLearnSequence(); renderLearnGrammar(); renderLearnVocab(); renderLearnVerbs(); }
function renderLearnSequence(){ const q=($('#learnSearch')?.value||'').toLowerCase(); const rows=LEARNING_PATH.filter(x=>!q || x.topic.toLowerCase().includes(q) || x.focus.toLowerCase().includes(q)); $('#learnSequenceWrap').innerHTML=rows.map((x,i)=>`<div class="learn-seq-card"><div><div class="badge">Step ${i+1}</div><h4>${x.topic}</h4><div class="muted">${x.focus}</div></div><div class="row gap8 wrap"><button class="ghost" data-learnpractice="${x.topic}">${t('Open lesson','Lektion öffnen')}</button></div></div>`).join(''); $$('#learnSequenceWrap [data-learnpractice]').forEach(b=>b.onclick=()=>{ goSection('practice'); switchPracticeGroup('grammar'); switchPractice('topicflow'); const sel=$('#topicFlowSelect'); if(sel) sel.value=b.dataset.learnpractice; newTopicFlow(); }); }
function renderLearnGrammar(){ const q=($('#learnGrammarSearch')?.value||'').toLowerCase(); const list=[]; const notes=state.data.notes||{}; Object.entries(notes).forEach(([k,v])=>{ if(!v) return; list.push({title:k, body: typeof v==='string'?v:(v.rule||v.text||JSON.stringify(v))}); }); const rows=list.filter(x=>!q || x.title.toLowerCase().includes(q) || x.body.toLowerCase().includes(q)); $('#learnGrammarWrap').innerHTML=rows.map(x=>`<details class="grammar-card"><summary>${x.title}</summary><div class="note-block">${x.body}</div></details>`).join('') || '<div class="note-block">No grammar data found.</div>'; }
function renderLearnVocab(){ const q=($('#learnVocabSearch')?.value||'').toLowerCase(); const showEn=$('#learnVocabEnglish')?.checked; const rows=state.data.vocab.filter(v=>!q || (v.de||v.word||'').toLowerCase().includes(q) || (v.en||v.translation||'').toLowerCase().includes(q)).slice(0,300); $('#learnVocabWrap').innerHTML=rows.map(v=>`<div class="list-item"><div><strong>${v.de||v.word||'—'}</strong>${showEn?` <span class="muted">— ${v.en||v.translation||''}</span>`:''}<div class="tiny muted">${v.topic||''}</div></div></div>`).join('') || '<div class="note-block">No vocabulary found.</div>'; }
function renderLearnVerbs(){ const q=($('#learnVerbSearch')?.value||'').toLowerCase(); const rows=[...(state.data.verbs||[]), ...(state.data.conjugation||[])].filter(v=>((v.verb||v.infinitive||'').toLowerCase().includes(q)||!q)).slice(0,300); $('#learnVerbWrap').innerHTML=rows.map(v=>`<div class="list-item"><div><strong>${v.verb||v.infinitive||'—'}</strong> <span class="muted">— ${v.translation||v.english||''}</span><div class="tiny muted">${v.level||''} ${v.topic||''}</div></div></div>`).join('') || '<div class="note-block">No verbs found.</div>'; }
function renderProgressSection(){ if($('#statsCenter')) $('#progressStats').innerHTML=$('#statsCenter').innerHTML; if($('#heatmapWrap')) $('#progressHeatmap').innerHTML=$('#heatmapWrap').innerHTML; if($('#badgesWrap')) $('#progressBadges').innerHTML=$('#badgesWrap').innerHTML; $('#progressWeak').textContent=getWeakAreas().slice(0,5).map(x=>`${x.type} (${x.wrong} wrong)`).join(' • ') || 'No weak areas tracked yet.'; }
function bind(){ $('#sidebarOpen').onclick=()=>$('#sidebar').classList.add('open'); $('#sidebarClose').onclick=()=>$('#sidebar').classList.remove('open'); $$('.navbtn').forEach(b=>b.onclick=()=>goSection(b.dataset.section)); $$('.learnbtn').forEach(b=>b.onclick=()=>switchLearn(b.dataset.learn)); $$('.pgroupbtn').forEach(b=>b.onclick=()=>switchPracticeGroup(b.dataset.pgroup)); $$('.psubbtn').forEach(b=>b.onclick=()=>switchPractice(b.dataset.practice)); $$('.vocabbtn').forEach(b=>b.onclick=()=>switchVocab(b.dataset.vmode)); $$('.exambtn').forEach(b=>b.onclick=()=>switchExam(b.dataset.emode)); $('#themeSelect').onchange=e=>setTheme(e.target.value); $('#settingsTheme').onchange=e=>setTheme(e.target.value); if($('#settingsTheme2')) $('#settingsTheme2').onchange=e=>setTheme(e.target.value); $('#languageSelect').onchange=e=>setLanguage(e.target.value); $('#settingsLanguage').onchange=e=>setLanguage(e.target.value); if($('#settingsLanguage2')) $('#settingsLanguage2').onchange=e=>setLanguage(e.target.value); $('#missionPreset').onchange=e=>{ store.missionPreset=e.target.value; saveStore(); updateHeader(); }; ['customPassages','customExercises','customVocab','customWriting'].forEach(id=>$('#'+id).onchange=()=>{ store.missionCustom={passages:+$('#customPassages').value,exercises:+$('#customExercises').value,vocab:+$('#customVocab').value,writing:+$('#customWriting').value}; saveStore(); updateHeader(); }); $('#continueBtn').onclick=()=>{ const weak=getWeakAreas()[0]; if(weak) openWeakTopic(weak.type); else { goSection('practice'); switchPractice('exercises'); newExercise(); } }; $('#resetTodayBtn').onclick=()=>{ store.daily[todayKey()]={uniquePassages:[],repeatPassages:0,correctExercises:0,attemptedExercises:0,uniqueVocab:[],writingDone:0,listeningDone:0,xp:0}; saveStore(); updateHeader(); }; $('#countRepeatsToggle').onchange=e=>{ store.countRepeats=e.target.checked; saveStore(); }; if($('#homeContinueBtn')) $('#homeContinueBtn').onclick=()=>{ goSection('practice'); switchPracticeGroup('grammar'); switchPractice('topicflow'); newTopicFlow(); }; if($('#homeWeakBtn')) $('#homeWeakBtn').onclick=()=>{ goSection('progress'); renderProgressSection(); }; if($('#learnSearch')) $('#learnSearch').oninput=renderLearnSequence; if($('#learnGrammarSearch')) $('#learnGrammarSearch').oninput=renderLearnGrammar; if($('#learnVocabSearch')) $('#learnVocabSearch').oninput=renderLearnVocab; if($('#learnVocabEnglish')) $('#learnVocabEnglish').onchange=renderLearnVocab; if($('#learnVerbSearch')) $('#learnVerbSearch').oninput=renderLearnVerbs; if($('#countRepeatsToggle2')) $('#countRepeatsToggle2').onchange=e=>{ store.countRepeats=e.target.checked; if($('#countRepeatsToggle')) $('#countRepeatsToggle').checked=e.target.checked; saveStore(); }; if($('#resetTodayBtn2')) $('#resetTodayBtn2').onclick=$('#resetTodayBtn').onclick; $('#exerciseLevelFilter').onchange=newExercise; $('#exerciseTypeFilter').onchange=newExercise; $('#exerciseFocusFilter').onchange=newExercise; $('#nextExerciseBtn').onclick=newExercise; $('#checkExerciseBtn').onclick=checkExercise; $('#showExerciseAnswerBtn').onclick=showExerciseAnswer; $('#typingLevelFilter').onchange=renderTypingOptions; $('#typingSelect').onchange=loadTypingPassage; $('#typingShowEnglish').onchange=()=>$('#typingEnglishWrap').classList.toggle('hidden',!$('#typingShowEnglish').checked); $('#playPassageBtn').onclick=()=>{ const p=currentPassage(); if(p) speak(p.german,$('#audioSpeed').value); }; $('#stopPassageBtn').onclick=stopSpeak; $('#repeatPassageBtn').onclick=()=>{ const p=currentPassage(); if(p) speak(p.german,$('#audioSpeed').value); }; $('#checkTypingBtn').onclick=checkTyping; $('#clearTypingBtn').onclick=()=>$('#typingInput').value=''; $('#listenLevelFilter').onchange=newListening; if($('#listenShowEnglish')) $('#listenShowEnglish').onchange=newListening; $('#newListeningBtn').onclick=newListening; $('#playListenBtn').onclick=()=>speak(state.currentListen.text,$('#listenSpeed').value); $('#stopListenBtn').onclick=stopSpeak; $('#replayListenBtn').onclick=()=>speak(state.currentListen.text,$('#listenSpeed').value); $('#revealListenBtn').onclick=()=>$('#listenAnswer').classList.remove('hidden'); $('#checkListenBtn').onclick=checkListening; $('#structureLevelFilter').onchange=newStructure; $('#newStructureBtn').onclick=newStructure; $('#checkStructureBtn').onclick=checkStructure; $('#showStructureBtn').onclick=showStructure; $('#caseLevelFilter').onchange=()=>newCaseDrill(); $('#newCaseBtn').onclick=()=>newCaseDrill(); $('#showCaseBtn').onclick=showCaseDrill; $('#writingTaskSelect').onchange=renderWritingTask; if($('#writingShowEnglish')) $('#writingShowEnglish').onchange=renderWritingTask; $('#newWritingBtn').onclick=()=>{ $('#writingTaskSelect').selectedIndex=($('#writingTaskSelect').selectedIndex+1)%WRITING_TASKS.length; renderWritingTask(); }; $('#checkWritingBtn').onclick=checkWriting; $('#showWritingSampleBtn').onclick=()=>$('#writingSample').classList.remove('hidden'); 
...
async function init(){
  ...
  updateUI(); goSection(store.lastSection||'home'); switchPracticeGroup(store.lastPracticeGroup||groupFromPractice(store.lastPractice||'topicflow')); switchPractice(store.lastPractice||'topicflow'); switchVocab('list'); switchExam('cefr');
